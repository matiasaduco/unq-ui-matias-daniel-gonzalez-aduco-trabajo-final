...
const SelectionDrawInterface = ({player, winner}) => {
    return(
        <div className="text-center">
            ...
            <img src={player.selection.name ? player.selection.img : incognito} alt="selección" className={`w-75 mt-2 mb-3 p-3 border rounded ${winner.name === player.name? 'border-3 border-success' : ''}`}/>
        </div>
    )
}
